...
fineTimeStep   = 1e-2; # data generation on fine scale
...
# sIdx = 30
# pp = m[(sIdx - 1) * numberOfDirect + 1: sIdx * numberOfDirect, 9].*fineTimeStep;
# plot(pp,linewidth=4)
# ax = gca()
# axis("tight")
# ylabel("traveltime")
...
# scatter([t[1],t[2]-1], pp[[t[1], t[2]-1]], s = 120, alpha=0.4)
...
# pp = m[(sIdx - 1) * numberOfDirect + 1: sIdx * numberOfDirect, 5:6].*fineTimeStep;
# zz = atan2.(pp[:,2],pp[:,1])
# n=length(zz)
# for i =2:length(zz)
...
# axis("tight")
# t = mod.(unbrokenRaysBound[(2*sIdx - 1):2 * sIdx],  numberOfDirect)
# scatter([t[1],t[2]-1], zz[[t[1], t[2]-1]], s = 120, alpha=0.4)
# ax[:tick_params]("both",labelsize=24)
# ax[:set_xlabel]("directions", fontsize=30)
# ax[:set_title]("exiting angles of 30th boundary point", fontsize=30)
# ax[:set_ylabel]("exiting angles", fontsize=30)

# pp = m[(sIdx - 1) * numberOfDirect + 1: sIdx * numberOfDirect, 7:8].*fineTimeStep;
# zz = atan2.(pp[:,2],pp[:,1])
# n=length(zz)
# for i =2:length(zz)
#     if abs(zz[i]-zz[i-1]) > 1.5 * pi
